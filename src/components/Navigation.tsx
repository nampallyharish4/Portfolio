...
const Navigation: React.FC = () => {
  ...
  const getNavItemClass = (sectionId: string) => {
    const baseClass =
      'relative px-6 py-3 font-medium transition-all duration-500 hover:scale-105 smooth-hover cursor-pointer group outline-none focus:outline-none glass-light rounded-xl hover:glass-medium';
    const isActive =
  ...
  const getMobileNavItemClass = (sectionId: string) => {
    const baseClass =
      'relative text-left p-4 transition-all duration-500 hover:scale-105 smooth-hover cursor-pointer group outline-none focus:outline-none glass-light rounded-xl hover:glass-medium';
    const isActive =
      activeSection === sectionId ||
      (sectionId === 'home' && activeSection === 'home');
  ...
  const UnderlineEffect = ({ isActive }: { isActive: boolean }) => (
    <div
      className={`absolute bottom-1 left-1/2 transform -translate-x-1/2 h-0.5 bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-400 dark:to-purple-400 transition-all duration-500 ${
        isActive
          ? 'w-4/5 opacity-100'
  ...
  const MobileUnderlineEffect = ({ isActive }: { isActive: boolean }) => (
    <div
      className={`absolute bottom-1 left-4 h-0.5 bg-gradient-to-r from-blue-600 to-purple-600 dark:from-blue-400 dark:to-purple-400 transition-all duration-500 ${
        isActive
          ? 'w-3/4 opacity-100'
          ...
          <div className="absolute right-0 flex items-center space-x-4">
            ...
            >
              <div className="relative w-6 h-6">
                <X
                  className={`absolute inset-0 w-6 h-6 text-gray-800 dark:text-gray-100 transition-all duration-500 ${
